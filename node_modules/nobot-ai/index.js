...
class NobotAi {
  ...
  async init() {
    const txtUserAgent = await fs.readFile("node_modules/nobot-ai/user_agent_blacklist.txt", "utf-8");
    this.word_user_agent_blacklist = txtUserAgent
      .trim()
      .split("\n")
      .map((word) => word.replace(/\r/g, ""));
    const txtIp = await fs.readFile("node_modules/nobot-ai/ip_blacklist.txt", "utf-8");
    this.ip_blacklist = txtIp
      .trim()
      .split("\n")
      .map((word) => word.replace(/\r/g, ""));
    const txtIsp = await fs.readFile("node_modules/nobot-ai/isp_blacklist.txt", "utf-8");
